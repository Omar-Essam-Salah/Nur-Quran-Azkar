  ...
  return (
    <div className="page-enter min-h-screen">
      ...
      {selected && (
        ...
        >
          {/* Centred card; if a description is long the CARD itself scrolls
              (capped at 85vh) instead of pushing content off-screen. */}
          <div
            className="max-w-sm w-full rounded-2xl text-center max-h-[85vh] overflow-y-auto custom-scrollbar p-6 space-y-2.5"
          ...
          >
            ...
            {extra[selected.n] && (
              <p className={`text-[13px] text-white/70 leading-relaxed border-t border-white/10 pt-3 ${isAr ? 'arabic-text' : ''}`} dir={isAr ? 'rtl' : 'ltr'}>
                {isAr ? extra[selected.n].exp : extra[selected.n].expEn}
              </p>
            )}
            <button onClick={() => setSelected(null)} className="glass-btn w-full py-2.5 text-sm mt-1">{t('Close', 'إغلاق')}</button>
          </div>
        </div>
      )}
    </div>
